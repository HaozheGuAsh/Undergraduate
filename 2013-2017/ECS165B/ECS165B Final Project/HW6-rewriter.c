...
"(PROJECTION (GROUP (TABLENAME $1) $2) $3) => (ITERATE (TABLENAME $1) (PROJECTROW(GROUP(GETNEXT (TABLENAME $1))$2) $3))",
"(PROJECTION (TABLENAME $1 ) $2) => (ITERATE (TABLENAME $1) (PROJECTROW (GETNEXT (TABLENAME $1)) $2))",
"(GROUP (ITERATE $1 $2) $3) => (ITERATE $1 (GROUP $2 $3))",
...
"(SELECTION (PRODUCT $1 $2) $3) => (SELECTION (TABLENAME (PRODUCT $1 $2)) $3) "
};
int rulecnt=7;
...
init_op_map()
{
	...
	i = OP_SORT; opmap[i].name = "SORT";		opmap[i].flag = IMPLEMENTED;
	i = OP_SORTSPEC; opmap[i].name = "SORTSPEC";		opmap[i].flag = IMPLEMENTED;
	...
	i = OP_DISTINCT; opmap[i].name = "DISTINCT";		opmap[i].flag = IMPLEMENTED;
	i = OP_AVG; opmap[i].name = "AVG";		opmap[i].flag = IMPLEMENTED;
	i = OP_COUNT; opmap[i].name = "COUNT";		opmap[i].flag = IMPLEMENTED;
	i = OP_ITERATE; opmap[i].name = "ITERATE";		 opmap[i].flag = IMPLEMENTED;
	i = OP_GETNEXT_ISCAN; opmap[i].name = "OP_GETNEXT_ISCAN"; opmap[i].flag = IMPLEMENTED;
	i = OP_RANGE; opmap[i].name = "OP_RANGE"; opmap[i].flag = IMPLEMENTED;
	i = OP_JOIN; opmap[i].name = "OP_JOIN"; opmap[i].flag = IMPLEMENTED;
	i = OP_ON; opmap[i].name = "OP_ON"; opmap[i].flag = IMPLEMENTED;
	init_rules();
...
opnum(char *str)
{
	...
	printf("::opnum: unrecognized OP %s\n", str);
	return 0;
}
	...
	if(fid) {
		while((c=fgetc(fid))>0) {
			if(c=='\n') {
				...
				if(!rules[rulecnt]) {
					printf("::init_rules: Out of memory, should never happen here\n");
					exit(-1);
				}
