...
data_insert insertdata;
data_select selectdata;
char datapath[1024];
int main()
{
	open_directory();
...
makeexpr(int op, int cnt, int arg1, int arg2)
{
	...
	//printf(":make_expr %x called with %d %d %d %d\n", ep,op, cnt, arg1, arg2);
...
print_e(expr e, int lev)
{
	...
	if(!ep) { printf("() "); return; }
	/*printf("func is : %d , count is : %d ,ep is %x\n", ep->func, ep->count, ep);
	if ((ep->values[0].ep)!=NULL&&ep->func!=OP_NUMBER)printf("union1: name:%s data:%s num:%d ep: %x\n "
		, ep->values[0].name, ep->values[0].data, ep->values[0].num, ep->values[0].ep);
	if ((ep->values[1].ep) != NULL&&ep->func != OP_NUMBER) printf("union2: name:%s data:%s num:%d ep: %x \n"
		,ep->values[1].name, ep->values[1].data, ep->values[1].num, ep->values[1].ep);
		*/
	switch(ep->func) {
