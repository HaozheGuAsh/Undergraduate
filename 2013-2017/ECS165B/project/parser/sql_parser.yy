...
%%
program
    :   sql_command { YYACCEPT; }
	|	QUIT ';'	{ YYABORT; }
	;

sql_command
    :	INSERT INTO tablename VALUES '(' data_list ')' ';'
		{   driver.evalexpr(driver.optimize(driver.compile(driver.cvt_itoe(driver.makeexpr(OP_INSERTROW, 2, $3, $6)))));
		    $$=0;
		    driver.clear();
		}
	...
	|	CREATE TABLE tablename '(' column_decls ')' ';'
		{   driver.evalexpr(driver.optimize(driver.compile(driver.cvt_itoe(driver.makeexpr(OP_CREATETABLE, 2, $3, $5)))));
		    $$=0;
		    driver.clear();
		}
	|	DESCRIBE tablename ';'
...
colname	:	IDENT { $$ = driver.setname(OP_COLNAME, $<str>1); }
    |	error ';'
...
orderlist:	orderspec               { $$ = driver.makeexpr(OP_RLIST, 2, $1, 0); }
...
orderspec:	NUMBER   { $$ = driver.makeexpr(OP_SORTSPEC, 2, $1, 0); }
	...
	|	colname      { $$ = driver.makeexpr(OP_SORTSPEC, 2, driver.setname(OP_COLNAME, $<str>1), 0); }
	|	colname ASC  { $$ = driver.makeexpr(OP_SORTSPEC, 2, driver.setname(OP_COLNAME, $<str>1), 0); }
	|	colname DESC { $$ = driver.makeexpr(OP_SORTSPEC, 2, driver.setname(OP_COLNAME, $<str>1), driver.makeexpr(OP_NULL, 0, 0, 0)); }
	;
...
join:	tablename           { $$ = $1; }
	|	join tablename  { $$ = driver.makeexpr(OP_PRODUCT, 2, $1, $2); }
	;
