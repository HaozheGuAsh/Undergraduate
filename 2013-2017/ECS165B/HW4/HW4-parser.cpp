...
int main()
{
	tables = open_directory();
	getcwd(datapath, sizeof(datapath));
	cout << "datapath is : " << datapath << "/database"<<endl;
    while (true) {
        if (!yyparse())  {
            break;
        }
...
print_e(expr e, int lev)
{
	...
	printf("func is : %d , count is : %d , union1: name:%s data:%s num:%d ep: %x , union2: name:%s data:%s num:%d ep: %x \n"
		, ep->func, ep->count,
