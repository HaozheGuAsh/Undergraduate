...
public class AStarDiv_999200555_912143385 implements AIModule
{
...
//System.out.println("HERE");
  ...
  //  double fn = gn+hn;
   ...
   {
      double fraction = Math.pow(Math.abs(htdif),1/dis);
      ...
      {
        return dis*(1/fraction);
      }
   }
*/
  //  return 0.5*(dis-2)+(1/(map.getTile(pt2)+1))+(map.getTile(map.getEndPoint())/2);
  //  return 0.5*(dis-2)+((map.getTile(pt2)/256))+(256/map.getTile(map.getEndPoint())+1);
    return 0.5*(dis-1)+(map.getTile(pt2))/(map.getTile(map.getEndPoint())+1);
    //return hn;
  }
