...
public class Ashminimax extends AIModule
{
    ...
    public void getNextMove(final GameStateModule state)
    {
        ...
        if(!terminate)
        {
            move= ChooseMove(state,maxdepth,-Integer.MAX_VALUE,Integer.MAX_VALUE).column;
    ...
    private mymove ChooseMove(final GameStateModule state, int depth,int low, int high)
    {
        ...
        for (int move = 0;move<state.getWidth();move++)
        {
            if(state.canMakeMove(move))
            {
               ...
               /* else if(player ==copy.getActivePlayer())//is it my turn? MAX:MIN, should not be here for two player
                ...
                {
                    currentmove = ChooseMove(copy,depth-1,-high,-low);
