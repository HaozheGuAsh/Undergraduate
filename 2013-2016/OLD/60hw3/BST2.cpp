//Mengyingzi Dai, 999098958, kimdai@ucdavis.edu
#include "BST2.h"
#include <iostream>
using namespace std;
...
{	if(x <= y){
	...
	printRangeTree(root,x,y);  
	cout<<endl;
	}else{
	//cout<<"x>y"<<endl;
	printRangeTree(root,y,x);
	cout<<endl;
	}
}	

template <class Comparable>
void BinarySearchTree<Comparable>::printRangeTree(BinaryNode<Comparable> *t, const Comparable &x, const Comparable &y) const
{	
	
	//cout<<"print Range Tree:"<<endl;
	if(t!=NULL){
		//cout<<"root is not null"<<endl;
		if(t->element >x){
			//cout<<"t>x"<<endl;
			printRangeTree(t->left,x,y);			
		}
