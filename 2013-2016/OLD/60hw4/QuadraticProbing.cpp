...
{         Human human1,human2;
          ...
          if(human.childCount>0)
          {
                for(int i =0; i<human.childCount;i++)
                {
                   strcpy(human2.person.lastName,((human.children)+i)->lastName);
		   strcpy(human2.person.firstName,((human.children)+i)->firstName);
		    human2.person.year=((human.children)+i)->year;
		    human2.person.gender=((human.children)+i)->gender;
                     human1 =find(human2);
		    cout<<"dddd"<<human1.index;
                    human1.parent=1;
                    if(human.person.gender=='M'){human1.dadindex=human.index;}
                    if(human.person.gender=='F'){human1.mumindex=human.index;}
                }
          }
