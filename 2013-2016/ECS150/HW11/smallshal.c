...
int where;
{
 ...
 // printf("in runcommand type is %d\n",where);
  if((pid = fork()) < 0)
  {
    perror("smallsh");
    return(-1);
...
main()
{
  ...
  sact.sa_handler=shell_action; 
  sigaction(SIGINT,&sact,&oacti);
  sigaction(SIGQUIT,&sact,&oactq);
  printf("shell pid is %d\n",getpid());
  while(userin(prompt) != EOF)
    procline();
}
void shell_action(int sig)
{
  char dummy;
  printf("shell pid is %d\n",getpid());
  ...
  if(curchild>0)
  {
   printf("Do you really want to kill process %d ?(type y to kill)",curchild);
   scanf("%c",&dummy);
   if(dummy=='y')
   {
