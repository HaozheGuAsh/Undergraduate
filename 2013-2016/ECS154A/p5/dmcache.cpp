...
int main(int argc, char * argv[])
{
	...
	strcpy(outfilename, "dm-out.txt");
	//strcat(outfilename, originalname.substr(6,originalname.size()-6).c_str());
	...
	while (input >> address >> operation >> data)
	{
		...
		{
			//cout << "write" << endl;
			if (cache[curline].tag == curtag&& cache[curline].used)
			{
				/*update*/
