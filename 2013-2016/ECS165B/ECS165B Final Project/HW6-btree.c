...
btree_lookupr(int index, int blknum, char *skey, char *ekey)
{
	...
	btree_ent *entp;
	int i, c=0;
	//printf("lookupr skey:%s \n",skey);
	if(!skey) return 0;
	bp = blkread(index, blknum);
	//printf("keynum %d", bp->keynum);
	if(0 == bp) {
	...
	for(i=1; i<=bp->keynum; i++) {
		entp = bp->entry+i;
		//printf("compare key skey: %s key: %s", skey, bp->data + (entp->keyoff));
	...
	}
	{
		...
		if((bp->keyuse - strlen(newkey) - 1) > (sizeof (btree_ent))*(bp->keynum+2)) {
			...
			if(i>1 && !strcmp(newkey, bp->data+bp->entry[i-1].keyoff)) {
				bp->entry[j].keyoff = bp->entry[i-1].keyoff;
			} else {
...
getdataloc(int iid, int ptr, char *ekey)
{
	...
	if (ptr < BLOCKSIZE / sizeof(btree_ent)) { printf("return smaller than begin\n"); return rv; }

	blknum = ptr / (BLOCKSIZE/sizeof (btree_ent));
	entidx = ptr % (BLOCKSIZE/sizeof (btree_ent));
	//printf("cursor ptr : %d ,blknum: %d  extidx %d", ptr,blknum,entidx);
	bp = blkread(iid, blknum);
	//printf("doing strcmp ekey: %s right: %s\n", ekey, bp->data + bp->entry[entidx].keyoff);
	//printf("compare data %d \n", strcmp(ekey, bp->data + bp->entry[entidx].keyoff));
	if (atoi(ekey) > atoi(bp->data + bp->entry[entidx].keyoff))  {
		//printf("in condition!!");
		rv.dloc = bp->entry[entidx].location;
		if (bp->keynum > entidx) 
		{ 
			rv.nptr = (blknum*(BLOCKSIZE / sizeof(btree_ent))) + entidx + 1;
			//printf("condition 1 ");
		}
		else if (bp->nextleaf) 
		{
			rv.nptr = (bp->nextleaf * (BLOCKSIZE / sizeof(btree_ent))) + 1;
			//printf("condition 2 ");
		}
		else
		{
	...
	case OP_COLNAME:	cursorp = (struct cursor *)colp->values[1].ep;
				...
				do {
					if(!cursorp) {
						...
						if (range && range->values[0].data)
						{
							skey = range->values[0].data; 
				/*			if (skey[0] == 0)
							{
								char ary[5];
								snprintf(ary, 10, "%d", 0);
						...
						//printf("!!!ekey %s", cursorp->ekey);
						//printf("::getnext_iscan: about to do lookup\n");
						//printf("cursor iid is %d", cursorp->iid);
					...
					//printf("for row iid: %d ptr: %d ekey: %s ", cursorp->iid, cursorp->ptr, cursorp->ekey);
					rv = getdataloc(cursorp->iid, cursorp->ptr, cursorp->ekey);
					//printf("::getnext_iscan: got cursor\n");
					//printf("rv got rv.dloc: %d rv.nptr:%d\n", rv.dloc, rv.nptr);
